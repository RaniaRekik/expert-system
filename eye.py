...
#Search in the rules which recommendation was redirected
def detectDisease():
...
def openNewWindow():
    ...
    tk.Label(newWindow, text ="Eye diseases").pack()
    ...
    img = ImageTk.PhotoImage(Image.open(photo).resize((400, 200)))

    canvas = tk.Canvas(newWindow, width=900, height=700)
    canvas.pack()
    canvas.create_image(400, 200, anchor=tk.CENTER, image=img)
    canvas.img = img
    
    # name diesease
    tk.Label(newWindow, text =str(name)).place(relx=0.5,
                       rely=0.5,
                       anchor='center')


    # description
    tk.Label(newWindow, text =str(description) ).place(relx=0.5,
                       rely=0.6,
                       anchor='center')
...
tk.Label(source, text="Check the options", width=30, bg=background,
         font=fontTipo).grid(row = 3, column = 1, pady = 0)
#################part1
#q1 itching
...
b1 = tk.Button(source, text="        Confirm        ", command = openNewWindow, bg=background)
b1.grid(row = 25, column = 1, padx = 10, pady=0)
